...
public class KeyValueTest extends BaseTest {
	...
	public void keyValueAccessTest() {
		...
		RBucketReactive<String> bucket = this.client.getBucket("alumno:1:name", StringCodec.INSTANCE);
	...
	public void keyValueExpireTest() {
		...
		RBucketReactive<String> bucket = this.client.getBucket("alumno:1:name", StringCodec.INSTANCE);
		Mono<Void> set = bucket.set("carlo", 30, TimeUnit.SECONDS);
		Mono<Void> get = bucket.get().doOnNext(System.out::println).then();
	...
	public void keyValueExtendExpireTest() {
		...
		RBucketReactive<String> bucket = this.client.getBucket("joffre:1:name", StringCodec.INSTANCE);
		...
		// extend
		sleep(5000);
		Mono <Boolean> mono = bucket.expire(30, TimeUnit.SECONDS);
